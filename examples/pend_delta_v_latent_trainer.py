#!/usr/bin/env python
"""
Delta-V training with latent trajectory loss.

Same as pend_delta_v_frozen_trainer.py but adds a direct latent-space
trajectory loss: encode observed frames with the frozen encoder, then
compare against ODE-predicted latent positions. This bypasses the
decoder, giving delta_V_net a much more direct gradient signal.

Loss = pixel_recon + alpha * latent_trajectory_loss + ...

Usage:
    python examples/pend_delta_v_latent_trainer.py \
        ...
        --name pend-delta-v-latent \
        --max_epochs 1000 --latent_alpha 1.0
"""
...
class DeltaVLatentModel(pl.LightningModule):
    """
    Perturbation identification with additive delta_V_net
    and latent trajectory supervision.

    ALL existing parameters are FROZEN (requires_grad=False).
    Only delta_V_net is trainable.

    Key difference from DeltaVFrozenModel: adds a latent trajectory
    loss that encodes all observed frames and compares directly with
    ODE-predicted latent positions, bypassing the decoder.
    """
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        # with encoder observations. Direct gradient to delta_V_net,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', default='pend-delta-v-latent', type=str)
    ...
    parser = DeltaVLatentModel.add_model_specific_args(parser)
