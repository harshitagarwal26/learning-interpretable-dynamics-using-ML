#!/usr/bin/env python
"""
Delta-V training via LoRA + latent trajectory loss.

Combines two improvements over the base delta_V_net approach:
1. LoRA: low-rank adaptation of V_net's weight matrices (internal feature modification)
2. Latent trajectory loss: encode observed frames with frozen encoder, compare
   directly with ODE-predicted latent positions, bypassing the decoder.

Loss = pixel_recon + kl + latent_alpha * latent_trajectory_loss

Usage:
    python examples/pend_delta_v_lora_latent_trainer.py \
        ...
        --name pend-delta-v-lora-latent \
        --max_epochs 1000 --lora_rank 8 --latent_alpha 1.0
"""
...
class DeltaVLoRALatentModel(pl.LightningModule):
    """
    Perturbation identification via LoRA adaptation of V_net
    with latent trajectory supervision.

    ALL existing parameters are FROZEN (requires_grad=False).
    Only LoRA parameters (lora_A, lora_B per layer) are trainable.

    Latent trajectory loss: encode all observed frames with frozen encoder,
    compare directly with ODE-predicted latent positions. This gives LoRA
    parameters a direct gradient signal bypassing the decoder.
    """
    ...
    def __init__(self, hparams, data_path=None, pretrained_ckpt=None):
        ...
        # Apply LoRA to V_net immediately (needed for load_from_checkpoint)
    ...
    def forward(self, X, u):
        ...
        # Encode first two frames for initial conditions
        ...
        # Encode ALL observed frames for latent trajectory targets
        with torch.no_grad():
            self.q_targets = []
            for t in range(T):
                q_m_t = self.recog_q_net(X[t].reshape(self.bs, d*d))
                q_m_t, _ = q_m_t.split([2, 1], dim=1)
                q_m_n_t = q_m_t / q_m_t.norm(dim=-1, keepdim=True)
                self.q_targets.append(q_m_n_t)
            self.q_targets = torch.stack(self.q_targets)  # [T, bs, 2]
        ...
        self.qT_unflat = self.qT  # [T, bs, 2] — keep for latent loss
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        # Latent trajectory loss: compare ODE-predicted latent positions
        # with encoder observations. Direct gradient to LoRA params,
        # bypassing the decoder entirely.
        latent_loss = F.mse_loss(self.qT_unflat, self.q_targets)

        lambda_ = self.current_epoch / 8000 if self.hparams.annealing else 1 / 100
        latent_alpha = getattr(self.hparams, 'latent_alpha', 1.0)

        loss = -lhood + kl_q + lambda_ * norm_penalty + latent_alpha * latent_loss
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument('--latent_alpha', default=1.0, type=float,
                            help='Weight for latent trajectory loss')
        return parser
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', default='pend-delta-v-lora-latent', type=str)
    ...
    parser = DeltaVLoRALatentModel.add_model_specific_args(parser)
