...
class Model(pl.LightningModule):
    ...
    def forward(self, X, u):
        ...
        # added for the case of frozen encoder/decoder to calculate dV to get dynamics
        z0_u.requires_grad_(True)
...
def main(args):
    checkpoint_path = os.path.join(PARENT_DIR, 'checkpoints', 'pend-lag-cavae-T_p=4-epoch=983-step=7871.ckpt')
    model = Model.load_from_checkpoint(checkpoint_path, T_pred=args.T_pred)
    model.data_path = os.path.join(PARENT_DIR, 'datasets', 'pendulum-gym-image-dataset-train-reverse-angle-perturbed-sin6.pkl')
    
    # Freeze Encoder
    for param in model.recog_q_net.parameters():
        param.requires_grad = False
        print(param.requires_grad)
    # Freeze Decoder
    for param in model.obs_net.parameters():
        param.requires_grad = False
        print(param.requires_grad)
    # Freeze Mass Matrix
    for param in model.ode.M_net.parameters():
        param.requires_grad = False
        print(param.requires_grad)
    # Freeze Control Matrix
    for param in model.ode.g_net.parameters():
        param.requires_grad = False
        
    # Ensure Physics is unfrozen (just in case)
    for param in model.ode.parameters():
        print(param.requires_grad)
        # param.requires_grad = True

    checkpoint_callback = ModelCheckpoint(monitor='monitor', 
                                          filepath=os.path.join(PARENT_DIR, 'logs', args.name, args.name + f'-T_p={args.T_pred}' + '-{epoch}-{step}'),
                                          save_top_k=1,
                                          save_last=True)
    ...
    # model = Model(hparams=args, data_path=os.path.join(PARENT_DIR, 'datasets', 'pendulum-gym-image-dataset-train-reverse-angle-perturbed-sin10.pkl'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--T_pred', default=10, type=int)
