#!/usr/bin/env python
"""
Retrain V_net on perturbed data with frozen encoder/decoder.

This loads a pre-trained unperturbed model, freezes encoder + decoder,
and retrains ONLY V_net, M_net, g_net on perturbed pendulum data.

Usage:
    conda run -n lagrangian_clean python examples/pend_retrain_frozen_encdec.py \
        --resume_from results/pend/pend-lag-cavae-T_p=4-epoch=983-step=7871.ckpt \
        --data_path datasets/pendulum-gym-image-dataset-train-reverse-angle-perturbed-medium.pkl \
        --name pend-retrain-medium \
...
class FrozenEncDecModel(pl.LightningModule):
    """
    Model with frozen encoder/decoder, only V_net/M_net/g_net are trainable.
    """
    def __init__(self, hparams, data_path=None, pretrained_ckpt=None):
        super(FrozenEncDecModel, self).__init__()
    ...
    def forward(self, X, u):
        ...
        theta = self.get_theta_inv(self.qT[:, 0], self.qT[:, 1], 0, 0, bs=T*self.bs)
    ...
    def configure_optimizers(self):
        # Only optimize unfrozen parameters (V_net, M_net, g_net)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', default='pend-retrain-medium', type=str)
    ...
    parser.add_argument('--resume_from', type=str, required=True,
                        help='Path to pre-trained unperturbed checkpoint')
    parser.add_argument('--data_path', type=str, required=True,
                        help='Path to perturbed training data')
    parser.set_defaults(homo_u=False, annealing=False)
    parser = Trainer.add_argparse_args(parser)
    parser = FrozenEncDecModel.add_model_specific_args(parser)
